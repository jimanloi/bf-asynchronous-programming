...
const preparedExtra = Promise.all(
  [
    extras.egg,
    extras.redPepper,
    extras.onion,
...
const theOrder = Promise.all([
  ...
  .then(([extras, lindasMeal, monicasMeal, ninasMeal, oliviasMeal]) => {
    const [egg, redPepper, onion, pineapple, mushrooms, broccoli] = extras;
    return Promise.all([
      addPreparedExtras(lindasMeal, [egg, redPepper, onion]),
      addPreparedExtras(monicasMeal, [pineapple]),
      Promise.resolve(ninasMeal),
      addPreparedExtras(oliviasMeal, [egg, mushrooms, pineapple, broccoli]),
    ]);
  })
